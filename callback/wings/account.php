...
class BVAccountCallback extends BVCallbackBase {
	...
	function updateInfo($args) {
		...
		if (array_key_exists('update_info', $args)) {
			...
			$result['update_info'] = array(
				"status" => WPCOMAccount::exists($this->settings, $args['update_info']['pubkey'])
			);
		}
	
		if (array_key_exists('update_api_key', $args)) {
			WPCOMAccount::updateApiPublicKey($this->settings, $args['update_api_key']['pubkey']);
			$result['update_api_key'] = array(
				"status" => $this->settings->getOption(WPCOMAccount::$api_public_key)
			);
		}
	...
	function process($request) {
		...
		case "addacc":
			WPCOMAccount::addAccount($settings, $params['public'], $params['secret']);
			$resp = array("status" => WPCOMAccount::exists($settings, $params['public']));
			break;
		case "rmacc":
			$resp = array("status" => WPCOMAccount::remove($settings, $params['public']));
			break;
		case "updt":
			$account->updateInfo($params);
			$resp = array("status" => WPCOMAccount::exists($settings, $params['pubkey']));
			break;
		case "updtapikey":
			WPCOMAccount::updateApiPublicKey($settings, $params['pubkey']);
			$resp = array("status" => $settings->getOption(WPCOMAccount::$api_public_key));
			break;
		case "rmbvscrt":
			$resp = array("status" => WPCOMRecover::deleteDefaultSecret($settings));
			break;
		case "rmbvkeys":
		...
		case "rmoldbvacc":
			...
			break;
		case "fetch":
			$accounts = WPCOMAccount::allAccounts($settings);
