...
`wp-admin/admin.php?page=wpcom-migration-connect` (under the plugin's menu; `manage_options` to view, the `administrator` role to act; single-site only). *Log in with WordPress.com* registers the site with WordPress.com through `automattic/jetpack-connection` and sends the user to WordPress.com to authorize; WordPress.com's authorize page sends them back to this screen (`redirect_after_auth`, with `skip_pricing` so it skips the Jetpack plans page). When that page retries a failed authorization it re-enters through `admin.php?page=jetpack&connect_url_redirect=true`, a URL it hardcodes; the screen answers that too, on `admin_page_access_denied`, so a site with a real Jetpack page keeps its own handler. Once connected the screen shows the WordPress.com login, the blog ID, a *Continue on WordPress.com* link (filter `wpcom_migration_continue_url`) and *Disconnect*. Deactivating the plugin disconnects.
