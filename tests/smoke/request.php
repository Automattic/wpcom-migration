...
if ( 4 !== $argc ) {
	wpcom_migration_smoke_fail( 'Usage: php tests/smoke/request.php <base-url> <built-plugin-dir> <scenario>' );
}
...
if ( ! is_file( $wpcom_migration_client_file ) ) {
	wpcom_migration_smoke_fail( 'Not a built plugin tree (no HMAC client): ' . $wpcom_migration_plugin_dir );
}
...
switch ( $wpcom_migration_scenario ) {
	...
	case 'closed':
		...
		$response = wpcom_migration_smoke_request( $wpcom_migration_endpoint, array() );
		wpcom_migration_smoke_expect_status( $response, 200 );
		if ( false === stripos( $response['body'], '<html' ) ) {
			wpcom_migration_smoke_fail( 'Unsigned request with the window closed did not return the front page: ' . substr( $response['body'], 0, 200 ) );
		}
		if ( isset( $response['headers']['access-control-allow-origin'] ) ) {
			wpcom_migration_smoke_fail( 'Unsigned request with the window closed carried export CORS headers.' );
		}
		if ( isset( $response['headers']['content-type'] ) && false !== stripos( $response['headers']['content-type'], 'json' ) ) {
			wpcom_migration_smoke_fail( 'Unsigned request with the window closed answered JSON.' );
		}
		...
		wpcom_migration_smoke_expect_status( $response, 409 );
		wpcom_migration_smoke_expect_error_json( $response, 409 );
		break;

	case 'secret-hash-deleted':
		$response = wpcom_migration_smoke_request( $wpcom_migration_endpoint, wpcom_migration_smoke_signed_headers( $wpcom_migration_secret ) );
		wpcom_migration_smoke_expect_status( $response, 503 );
		wpcom_migration_smoke_expect_error_json( $response, 503 );
		break;
	...
	default:
		wpcom_migration_smoke_fail( 'Unknown scenario: ' . $wpcom_migration_scenario );
}
 ...
 * @return string[]
 */
function wpcom_migration_smoke_signed_headers( $secret ) {
 ...
 */
function wpcom_migration_smoke_request( $url, array $headers ) {
	...
	if ( false === $body ) {
		wpcom_migration_smoke_fail( 'Request failed: ' . $url );
	}
 ...
 *
 * @param array $response Response from wpcom_migration_smoke_request().
 * @param int   $expected Expected status code.
 */
function wpcom_migration_smoke_expect_status( array $response, $expected ) {
	if ( $expected !== $response['status'] ) {
		wpcom_migration_smoke_fail( sprintf( 'Expected HTTP %d, got %d: %s', $expected, $response['status'], substr( $response['body'], 0, 300 ) ) );
	}
}
 ...
 */
function wpcom_migration_smoke_expect_header( array $response, $name, $expected ) {
	if ( ! isset( $response['headers'][ $name ] ) || $expected !== $response['headers'][ $name ] ) {
		wpcom_migration_smoke_fail( sprintf( 'Expected header %s: %s, got: %s', $name, $expected, isset( $response['headers'][ $name ] ) ? $response['headers'][ $name ] : '(absent)' ) );
	}
}
 ...
 * @return array
 */
function wpcom_migration_smoke_expect_json( array $response ) {
	$json = json_decode( $response['body'], true );
	if ( ! is_array( $json ) ) {
		wpcom_migration_smoke_fail( 'Expected a JSON body, got: ' . substr( $response['body'], 0, 300 ) );
	}
	return $json;
}
 ...
 */
function wpcom_migration_smoke_expect_error_json( array $response, $code ) {
	$json = wpcom_migration_smoke_expect_json( $response );
	if ( ! isset( $json['code'], $json['error'] ) || $code !== (int) $json['code'] ) {
		wpcom_migration_smoke_fail( sprintf( 'Expected error JSON with code %d, got: %s', $code, $response['body'] ) );
	}
}
 ...
 */
function wpcom_migration_smoke_fail( $message ) {
